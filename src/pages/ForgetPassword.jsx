...
function ForgetPassword(props) {
  ...
  const handleSubmit = async (values, { setSubmitting, setErrors }) => {
    try {
      const response = await axios.post("https://backendhost-production-1804.up.railway.app/auth/request-otp", {
        email: values.email,
      },
      {headers: {
          ...
          >
            {({ errors }) => (
              <Form>
                ...
                />
                <ErrorMessage name="email" component="div" className="text-red-900 mt-1" />

                {errors.general && <div className="text-red-900 mt-2">{errors.general}</div>}

                <button type="submit" className="py-3 px-5 mt-6 bg-white text-[#FF6F00] font-medium rounded-md float-right">
                  Continue
                </button>
              </Form>
            )}
